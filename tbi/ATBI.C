...
idata    unsigned char ad_ch;        /*  A/D通道号   */
    ...
    code setup_index prompt[10]={
        ...
        {3,"\aCURRENT DATE:\n %d-%d-%d",0,0,NULL,(unsigned int*)&now.mon,(unsigned int *)&now.day,(unsigned int*)&now.year},
        {2,"\aCURRENT TIME:\n %d:%d:%d",0,0,NULL,(unsigned int*)&now.hour,(unsigned int*)&now.min,(unsigned int*)&sec},
        {1,"\aAUTO WORK BY\nTIME:%s",1,0,pmode,(unsigned int*)&mode,NULL,NULL},
        {2,"\aSTART TIME:\n %d:%d",0,0,NULL,(unsigned int*)&shour,(unsigned int*)&smin,NULL},
        {2,"\aEND TIME:\n %d:%d",0,0,NULL,(unsigned int*)&ehour,(unsigned int*)&emin,NULL},
...
/*  发送中断清除发送标志    */
/*--------------------------*/
interrupt [0x23] void serial(void)
{
    if(RI){
        buf[bufin]=SBUF;
...
main()
{
    ...
    set_eprom(EWEN);
    for(i=0;i<33;i++){
      write_eprom(i*4,600-i*10);
      write_eprom(i*4+1,i*10);
      write_eprom(i*4+2,2400-i*10);
      write_eprom(i*4+3,i*10);
      }
      set_eprom(EWDS);
    now.solar_state=1;
    dsw(0xa,0x2f);
